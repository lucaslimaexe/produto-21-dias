...
export interface BehavioralAnalysisData {
  archetype: string;
  summary: string; // Este será dividido em 3 cards
  keywords: string[];
  idealPercentage: number;
  missingForIdeal: string; // Este também pode virar um card ou parte do resumo
}
...
// Dados para Cards de Análise (derivados do summary e missingForIdeal)
const analysisCardsData = (analysisResult?: BehavioralAnalysisData) => {
    if (!analysisResult) return [];
    // Simplificando: pegando frases do summary e missingForIdeal. Idealmente, seriam mais elaboradas.
...
// Componente para Card de Análise
...
// Componente para Card de Módulo
    ...
    <Card className={cn("bg-slate-800/70 border-purple-600/80 hover:border-accent hover:shadow-accent/30 transition-all duration-300 transform hover:scale-105", className)}>
        <CardHeader className="flex flex-row items-center gap-3 p-4">
            ...
            <div>
                ...
                <p className="text-xs text-yellow-400/80 font-medium">Valor real: <span className="line-through">R${value.toFixed(2).replace('.', ',')}</span></p>
            </div>
        </CardHeader>
...
export const ResultsScreen: React.FC<ResultsScreenProps> = ({
  ...
  analysisError,
  userName,
  userDreams, // Já recebemos isso
  dreamsAchievementDateLabel // E isso
}) => {
  const [priceCardTimeLeft, setPriceCardTimeLeft] = useState(7 * 60); // 7 minutos para o card de preço
  ...
  const finalOfferTimerInitial = 2 * 60; // 2 minutos para a oferta final revelada
  ...
  // Efeito para toasty inicial
  useEffect(() => {
    if (analysisResult && !analysisError) {
      toast({
        title: `ALERTA MÁXIMO, ${displayName}!`,
        description: "Seu diagnóstico CRÍTICO chegou. Veja o que te impede AGORA.",
        variant: "destructive",
        duration: 7000,
      });
    } else if (analysisError) {
       toast({ title: "⚠️ Erro na Análise", description: analysisError || "Não foi possível carregar sua análise.", variant: "destructive", duration: 7000 });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Timer para o card de preço (7 minutos)
  useEffect(() => {
  ...
  // Mudança de vagas no card de preço após 5 minutos
  useEffect(() => {
    if (priceCardTimeLeft === (7*60 - 5*60)) { // Após 5 minutos (ou seja, quando restam 2 min)
        setPriceCardVacancies(1);
        playSound('limit_reached.mp3');
    }
  }, [priceCardTimeLeft]);

  // Timer para a oferta final revelada (2 minutos)
  useEffect(() => {
    ...
    if (finalOfferTimeLeft > 0 && finalOfferTimeLeft <= 60) { // Piscar no último minuto
  ...
  const handleRevealPrice = () => {
    ...
    playSound('dream_select.mp3'); // TODO: Usar som de revelação
    setFinalOfferTimeLeft(finalOfferTimerInitial); // Reseta o timer da oferta final
  };
  ...
  const DefaultContent: React.FC = () => (
    <div className="text-center py-10">
        ...
        <h2 className="font-headline text-3xl text-primary mb-2">Prepare-se para a Revelação!</h2>
        <p className="text-lg text-muted-foreground">Sua análise está sendo processada...</p>
    </div>
  );
  ...
  const currentAnalysisResult = analysisResult || {
      ...
      idealPercentage: 50, // Valor padrão
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center pt-16 sm:pt-20 pb-28 bg-gradient-to-br from-purple-950 via-black to-red-950 text-foreground overflow-x-hidden">
        ...
        <main className="w-full max-w-5xl space-y-12 md:space-y-16 px-4">
            {/* Bloco 1.2 e 1.3: Arquétipo, Nível de Alinhamento, Travas */}
            <section className="animate-fade-in text-center" style={{animationDuration: '0.7s'}}>
                <div className="grid md:grid-cols-2 gap-6 md:gap-8 items-center mb-8">
                    <Card className="bg-black/50 border-2 border-pink-500/70 p-6 rounded-2xl shadow-xl">
                        ...
                        <p className="text-2xl sm:text-3xl md:text-4xl font-bold text-pink-400 mb-0">{currentAnalysisResult.archetype}</p>
                    </Card>
                    <Card className="bg-black/50 border-2 border-yellow-500/70 p-6 rounded-2xl shadow-xl">
                        ...
                        <div className="flex items-center justify-center gap-3 mb-2">
                            <p className={cn("text-4xl sm:text-5xl md:text-6xl font-bold", getPercentageColor(currentAnalysisResult.idealPercentage).replace('bg-','text-'))}>{currentAnalysisResult.idealPercentage}%</p>
                                ...
                                <Tooltip delayDuration={100}>
                                    <TooltipTrigger asChild>
                                        <Badge variant="outline" className="text-sm bg-red-700/60 text-yellow-200 border-red-500/80 px-3 py-1.5 cursor-default animate-subtle-pulse" style={{animationDelay: `${index * 0.1}s`}}>
                                            <Lock className="h-3 w-3 mr-1.5" /> {keyword}
                                        </Badge>
                                    </TooltipTrigger>
                ...
                {/* Bloco 1.4: Textão de análise em carrossel */}
                <div className="mb-8">
                ...
                <Button onClick={() => document.getElementById('price-card-section')?.scrollIntoView({ behavior: 'smooth' })} className="goddess-gradient text-primary-foreground font-bold text-lg sm:text-xl py-3 sm:py-4 px-8 sm:px-10 rounded-xl shadow-xl hover:scale-105 transition-transform duration-300 animate-pulse-goddess">
                    <Unlock className="mr-2 h-5 w-5" /> DESBLOQUEAR MEU CÓDIGO
                </Button>
            </section>
            ...
            <section className="animate-fade-in space-y-6 text-center" style={{animationDelay: '0.5s'}}>
                ...
                <p className="text-md sm:text-lg text-purple-200/90 max-w-2xl mx-auto leading-relaxed animate-fade-in" style={{animationDelay: '0.8s'}}>
                    A verdade é que existe um <span className="text-red-400 font-semibold text-lg sm:text-xl">BLOQUEIO INVISÍVEL</span>. Mas a chave para sua liberdade está aqui.
                </p>
                <Button onClick={() => document.getElementById('modules-section')?.scrollIntoView({ behavior: 'smooth' })} variant="outline" className="border-accent text-accent hover:bg-accent/20 hover:text-yellow-300 font-semibold text-md sm:text-lg py-2.5 px-6 rounded-lg animate-fade-in animate-icon-subtle-float" style={{animationDelay: '1s'}}>
                    <Eye className="mr-2 h-5 w-5" /> Veja o que está incluído
                </Button>
            </section>

            <hr className="border-purple-700/30 my-10 md:my-14" />
            
            {/* Bloco 3: Ancoragem de Preço (Card de Preço) */}
                ...
                <Card className="max-w-md mx-auto bg-gradient-to-br from-primary/20 via-black to-accent/20 border-2 border-accent/70 p-6 sm:p-8 rounded-3xl shadow-2xl shadow-accent/30">
                    ...
                    <CardContent className="p-0 space-y-4">
                        <p className="text-xl sm:text-2xl text-purple-200/90">
                            Valor Total dos Módulos: <span className="line-through text-red-400/80 text-2xl sm:text-3xl font-semibold">R$ {totalRealValue.toFixed(2).replace('.',',')}</span>
                        </p>
                        ...
                        <div className="text-sm text-yellow-400/90 space-y-1">
                            <p className="animate-subtle-pulse"><TimerIcon className="inline h-4 w-4 mr-1" /> Promoção válida por: <span className="font-bold">{formatTime(priceCardTimeLeft)}</span></p>
                            <p className={priceCardVacancies === 1 ? 'text-red-400 font-bold animate-intense-pulse' : ''}><Users className="inline h-4 w-4 mr-1" /> Restam apenas: <span className="font-bold">{priceCardVacancies}</span> {priceCardVacancies === 1 ? 'acesso com este valor!' : 'acessos com este valor!'}</p>
                        </div>
                        <Button onClick={handleRevealPrice} disabled={isPriceRevealed} className={cn("w-full goddess-gradient text-primary-foreground font-bold text-lg sm:text-xl py-3 sm:py-4 rounded-xl shadow-xl hover:scale-105 transition-transform duration-300 mt-4", isPriceRevealed && "opacity-50 cursor-not-allowed")}>
                            <ShoppingCart className="mr-2 h-5 w-5" /> QUERO GARANTIR AGORA POR R$97
                        </Button>
                    </CardContent>
                </Card>
            </section>
            
            {/* Revelação do Preço Final e CTA Principal */}
            {isPriceRevealed && (
                <section id="final-offer-section" className="animate-pop-in bg-black/50 border-2 border-yellow-500 p-6 sm:p-10 rounded-3xl shadow-2xl shadow-yellow-500/50 text-center mt-10">
                    ...
                    <p className="text-purple-200/90 text-md sm:text-lg mb-6 max-w-xl mx-auto">
                        {displayName}, como uma oportunidade <span className="text-yellow-300 font-bold">ÚNICA E MÁGICA</span>, seu acesso a todo o CÓDIGO DA DEUSA™ é ainda mais especial:
                    </p>
                    ...
                    <p className="text-yellow-300 font-bold text-lg sm:text-xl md:text-2xl mb-6">
                        SIM! APENAS R$ {offerPriceFinal.toFixed(2).replace('.',',')} HOJE! <br className="sm:hidden"/> Um presente para sua transformação!
                    </p>
                    ...
                    >
                        <a href="https://pay.kiwify.com.br/xxxxxxxx" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2">
                            ...
                            <span className="leading-tight break-words">{finalOfferTimeLeft > 0 ? `EU QUERO O CÓDIGO POR R$47 AGORA!` : "OFERTA EXPIRADA"}</span>
            ...
            {/* Bloco 4: Conteúdo do Método (Módulos) */}
            <section id="modules-section" className="animate-fade-in" style={{animationDelay: '1.5s'}}>
                ...
                <div className="text-center mt-10">
                    <Button onClick={() => document.getElementById(isPriceRevealed ? 'final-offer-section' : 'price-card-section')?.scrollIntoView({ behavior: 'smooth' })} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-lg sm:text-xl py-3 sm:py-4 px-8 sm:px-10 rounded-xl shadow-xl hover:scale-105 transition-transform duration-300 animate-icon-subtle-float">
                        <Gift className="mr-2 h-5 w-5" /> QUERO ACESSAR TUDO ISSO
                    </Button>
                </div>
            </section>
            ...
            {/* Bloco 6: Comparativo Decisão Binária */}
            <section className="animate-fade-in" style={{animationDelay: '2.1s'}} onMouseEnter={handleScrollLock}>
                ...
                <div className={cn("grid md:grid-cols-2 gap-6 md:gap-8 transition-opacity duration-500", isScrollLocked && "opacity-50 blur-sm scroll-lock-pulse")}>
                    <Card className="bg-red-900/70 border-2 border-red-600 p-6 rounded-2xl">
                        <CardHeader className="p-0 mb-3 text-center">
                            <ThumbsDown className="h-10 w-10 text-red-300 mx-auto mb-2" />
                    ...
                    <Card className="bg-green-900/70 border-2 border-green-600 p-6 rounded-2xl">
                        <CardHeader className="p-0 mb-3 text-center">
                            <ThumbsUp className="h-10 w-10 text-green-300 mx-auto mb-2" />
                ...
                <div className={cn("text-center mt-10 transition-opacity duration-1000", isScrollLocked ? "opacity-0" : "opacity-100 animate-fade-in")} style={{animationDelay: isScrollLocked ? '0s' : '2s'}}>
                    <Button onClick={() => document.getElementById(isPriceRevealed ? 'final-offer-section' : 'price-card-section')?.scrollIntoView({ behavior: 'smooth' })} size="lg" className="bg-gradient-to-r from-purple-600 via-pink-600 to-red-600 hover:from-purple-700 hover:via-pink-700 hover:to-red-700 text-white font-bold text-lg sm:text-xl py-4 px-10 rounded-xl shadow-2xl animate-intense-pulse">
                        <Rocket className="mr-2 h-6 w-6" /> EU DECIDO VIRAR O JOGO!
                    </Button>
                </div>
            </section>
                ...
                <AlertDialog open={showRecusePopup} onOpenChange={setShowRecusePopup}>
                    <AlertDialogTrigger asChild>
                        <Button variant="ghost" className="text-purple-400/70 hover:text-red-400 hover:bg-red-900/30 text-sm">
                            <XCircle className="mr-2 h-4 w-4" /> Prefiro continuar como estou e perder essa chance.
                        </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent className="bg-slate-900 border-red-500 text-white">
                        ...
                        <AlertDialogFooter>
                            ...
                            <AlertDialogAction onClick={() => { setShowRecusePopup(false); playSound('form_error.mp3'); /* Idealmente, redirecionar ou logar a recusa */ }} className="bg-red-600 hover:bg-red-700 text-white">Sim, tenho certeza (e aceito as consequências)</AlertDialogAction>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                </AlertDialog>
            </section>
        ...
        <div className="fixed bottom-0 left-0 right-0 md:hidden bg-black/80 backdrop-blur-sm p-3 border-t border-purple-700/50 z-50 shadow-2xl animate-fade-in" style={{animationDelay: '3s'}}>
            <Button onClick={() => document.getElementById(isPriceRevealed ? 'final-offer-section' : 'price-card-section')?.scrollIntoView({ behavior: 'smooth' })} className="w-full goddess-gradient text-primary-foreground font-bold text-md py-3 rounded-lg animate-subtle-glow">
                <LucideSparkles className="mr-2 h-5 w-5 animate-ping absolute left-4 opacity-50" style={{animationDuration:'3s'}} />
                {stickyMessages[stickyMessageIndex]}
                <LucideSparkles className="ml-2 h-5 w-5 animate-ping absolute right-4 opacity-50" style={{animationDuration:'3s', animationDelay:'0.5s'}}/>
            </Button>
        </div>
    </div>
  );
...
// Adicionando o ícone TimerIcon que pode estar faltando
